...
if __name__ == '__main__':  # Точка входа при запуске этого скрипта
    ...
    print(f'Подключено к терминалу QUIK по адресу: {qp_provider.Host}:{qp_provider.RequestsPort},'
          f'{qp_provider.CallbacksPort}')

    # Проверяем соединение
    print(f'Терминал QUIK подключен к серверу: {qp_provider.IsConnected()["data"] == 1}')
    print(f'Отклик QUIK на команду Ping: {qp_provider.Ping()["data"]}')  # Проверка работы скрипта QuikSharp.
    # Должен вернуть Pong
    ...
    print(f'Отправка сообщения в QUIK: {msg}{qp_provider.MessageInfo(msg)["data"]}')  # Проверка работы QUIK. Сообщение
    # в QUIK должно показаться как информационное
    ...
    qp_provider.OnParam = lambda data: print(data)  # Текущие параметры изменяются постоянно.
    # Будем их смотреть, пока не нажмем Enter в консоли
