...
class MacdRsiStochStrategy(bt.Strategy):
    ...
    params = (
        ...
        ('macd_2_1', 12),  # быстрая скользящая средняя (индикатор MACD)
        ('macd_2_2', 26),  # медленная скользящая средняя (индикатор MACD)
...
if __name__ == '__main__':  # Точка входа при запуске этого скрипта
    ...
    cerebro.optstrategy(MacdRsiStochStrategy, macd_2_1=range(5, 20),
                        macd_2_2=range(20, 35),
                        macdsig_2_1=range(5, 20))  # Торговая система на оптимизацию с параметрами.
    ...
    for result in results:  # Пробегаемся по статистике по всем параметрам
        p = (result[0].p.macd_2_1, result[0].p.macd_2_2, result[0].p.macdsig_2_1) # Параметр
    ...
    print(f'Лучшие значения: MACD({bestStat[0]}), {bestStat[1]:.2f}')
    print(f'Худшие значени: MACD({worstStat[0]}), {worstStat[1]:.2f}')
