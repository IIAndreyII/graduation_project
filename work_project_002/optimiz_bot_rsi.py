...
if __name__ == '__main__':  # Точка входа при запуске этого скрипта
    ...
    cerebro.optstrategy(MacdRsiStochStrategy, rsiperiod=range(5, 20))  # Торговая система на оптимизацию с параметрами.
    ...
    for result in results:  # Пробегаемся по статистике по всем параметрам
        p = (result[0].p.rsiperiod)  # Параметр
    ...
    print(f'Лучшие значения: RSI({bestStat[0]}), {bestStat[1]:.2f}')
    print(f'Худшие значени: RSI({worstStat[0]}), {worstStat[1]:.2f}')
